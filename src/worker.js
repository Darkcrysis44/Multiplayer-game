...
export class GameRoom {
  ...
  async fetch(request) {
    ...
    this.players.set(id, { id, name, x: 300 + this.clients.size * 100, y: 300, hp: 100 });
    if (!this.enemies.length) this.enemies = [
      { id: 1, x: 600, y: 300, hp: 100 },
      { id: 2, x: 750, y: 450, hp: 100 },
      { id: 3, x: 450, y: 500, hp: 100 }
    ];
    server.send(JSON.stringify({ type: 'welcome', id, room: url.searchParams.get('room') }));
    this.broadcast();
    ...
    server.addEventListener('close', () => {
      ...
      this.broadcast();
      if (!this.clients.size && this.tick) { clearInterval(this.tick); this.tick = null; }
    });
  ...
  onMessage(id, data) {
    ...
    if (!p) return;
    if (m.type === 'move') {
      p.x = Math.max(20, Math.min(1180, Number(m.x) || p.x));
      p.y = Math.max(80, Math.min(680, Number(m.y) || p.y));
    }
    if (m.type === 'attack') {
      const e = this.enemies.find(e => Math.hypot(e.x - p.x, e.y - p.y) < 90);
      if (e) e.hp -= 25;
      if (e && e.hp <= 0) this.enemies = this.enemies.filter(x => x.id !== e.id);
    }
  }
  broadcast() {
    ...
    for (const ws of this.clients.values()) {
      try { ws.send(JSON.stringify({ type: 'state', players, enemies })); } catch {}
    }
  }
}
